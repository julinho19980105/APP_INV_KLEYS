...
import { collection, query, orderBy, addDoc, serverTimestamp, setDoc, doc, limit, getDocs, deleteDoc } from "firebase/firestore"
    ...
    <div className="space-y-6 pt-2 pb-24 px-2 md:px-0 max-w-4xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] ml-1 mt-1">Gestión Industrial Diva</p>
        </div>
        <div className="flex w-full md:w-auto gap-3">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="rounded-[2.5rem] border-none shadow-2xl max-w-md">
              <DialogHeader>
                <DialogTitle className="text-sm font-black text-black uppercase tracking-widest">Alta de Cliente</DialogTitle>
              </DialogHeader>
              <div className="space-y-4 pt-4">
                ...
                <div className="space-y-1">
                  <Label className="text-[9px] font-black uppercase text-black ml-1">Teléfono</Label>
                  <Input 
                ...
                <div className="space-y-1">
                  <Label className="text-[9px] font-black uppercase text-black ml-1">Ubicación</Label>
                  <Input 
                ...
                >
                  {saving ? <Loader2 className="w-5 h-5 animate-spin" /> : "REGISTRAR CLIENTE"}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
            ...
            {filteredCustomers.map(c => (
              <TableRow key={c.id} className="hover:bg-black/5 transition-colors group border-b last:border-0">
                <TableCell className="pl-8">
                  <span className="font-black text-[9px] bg-black text-white px-2 py-0.5 rounded-lg">{c.id}</span>
                </TableCell>
              ...
              <TableRow>
                <TableCell colSpan={3} className="text-center py-24">
                  <div className="flex flex-col items-center gap-3 opacity-20">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Sin clientes registrados</span>
                  </div>
                </TableCell>
              </TableRow>
            )}
