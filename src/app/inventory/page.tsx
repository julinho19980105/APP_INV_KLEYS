  ...
  return (
    <TableRow key={p.id} className="group transition-colors hover:bg-black/5">
      ...
      <TableCell className="text-right p-2">
        {confirming ? (
          <div className="flex items-center justify-end gap-1 animate-in slide-in-from-right-2 duration-200">
            <span className="text-[7px] font-black text-destructive uppercase mr-1">¿BAJA?</span>
            <Button 
              size="icon" 
              variant="ghost" 
              className="h-7 w-7 rounded-lg bg-destructive text-white hover:bg-destructive/90"
              onClick={() => {
                onDelete({ id: p.id, code: p.code });
                setConfirming(false);
              }}
            >
...
export default function InventoryPage() {
  ...
  const onDelete = React.useCallback((prod: {id: string, code: string}) => {
    if (!db) return
    ...
    const qIn = query(movementsRef, where("productCode", "==", prod.code), where("type", "in", ["in", "return"]))
