...
export default function QuotesPage() {
  ...
  const handleApplyCalc = () => {
    ...
    setCurrentEntry({ ...currentEntry, quantity: total.toString(), description: desc + (currentEntry.manualNote || "") })
    setIsCalcOpen(false)
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-24 pt-2 px-2 md:px-0">
      <div className="flex justify-between items-end border-b-2 border-black pb-4">
        ...
        {editId && (
          <Button variant="ghost" size="icon" className="h-10 w-10 text-destructive rounded-xl" onClick={() => router.push('/sales')}>
            <X className="w-6 h-6" />
          </Button>
        )}
      </div>
      ...
      <Card className="rounded-[2rem] border-2 border-black/5 bg-white overflow-hidden shadow-xl">
        <div className="bg-black/5 border-b py-3 px-8 flex justify-between items-center">
          <span className="text-[10px] font-black uppercase text-black/40 tracking-widest">PREPARACIÓN DE PRENDA</span>
          {currentEntry.name && (
        ...
        <CardContent className="p-4 md:p-8 space-y-6">
          {currentEntry.name && (
            <div className="border-b pb-4">
              ...
              <span className="text-[8px] font-black text-black/40 uppercase tracking-widest">{currentEntry.productId}</span>
            </div>
          )}
                ...
                <div className="grid grid-cols-2 gap-2 md:col-span-2">
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button variant="outline" size="icon" className="h-10 w-10 shrink-0 rounded-xl" onClick={() => setIsCalcOpen(true)}>
          ...
          <Button className="w-full h-12 bg-black text-white rounded-2xl font-black text-xs uppercase" onClick={addCurrentToList} disabled={!currentEntry.name}>
      ...
      <Card className="rounded-[2rem] border shadow-sm bg-white overflow-hidden">
        ...
        <div className="divide-y divide-black/5">
          {items.map((item) => (
            <div key={item.id} className="p-4 md:px-8 hover:bg-black/[0.01] transition-colors relative">
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-2">
                  <span className="font-black text-[12px] text-black uppercase">{item.name}</span>
                  <span className="text-[8px] font-black px-2 py-0.5 bg-black/5 rounded uppercase">{item.productId}</span>
                </div>
              ...
              <div className="flex justify-between items-end mt-1">
                <div className="text-[9px] font-normal uppercase text-black/50 space-x-2">
                  <span className="font-black text-black/70">{item.description}</span>
                  <span>|</span>
                  <span className="font-black" style={{ color: brandColor }}>{item.quantity} UND</span>
                  <span>× S/ {item.price}</span>
                  {Number(item.discount) > 0 && <span className="text-red-600 font-black"> - S/ {item.discount} DESC</span>}
                </div>
                <Button variant="ghost" size="icon" className="h-7 w-7 text-black/20 hover:text-destructive" onClick={() => setItems(items.filter(i => i.id !== item.id))}>
      ...
      <Dialog open={isCalcOpen} onOpenChange={setIsCalcOpen}>
        <DialogContent className="rounded-[2.5rem] border-none shadow-2xl max-w-xs">
          ...
          <div className="space-y-4 pt-4">
            ...
            <Button className="w-full h-12 bg-black text-white rounded-xl font-black text-[10px] uppercase" onClick={handleApplyCalc}>CONFIRMAR CÁLCULO</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
