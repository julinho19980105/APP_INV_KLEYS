            ...
            <CardContent className="p-1">
              <Accordion type="single" collapsible className="w-full space-y-1">
                {(logData?.entries || []).sort((a: any, b: any) => new Date(a.addedAt).getTime() - new Date(b.addedAt).getTime()).map((entry: any, index: number) => {
                  const balance = calculateEntryBalance(entry)
                  return (
                    <AccordionItem key={entry.customerId} value={entry.customerId} className="border border-primary/10 rounded-lg overflow-hidden bg-white shadow-sm">
                      <div className="flex items-center w-full px-2">
                        ...
                        <AccordionTrigger className="flex-1 hover:no-underline py-2.5 group">
                          <div className="flex items-center w-full">
                            ...
                            <div className={cn(
                              "ml-auto mr-2 px-2.5 py-1 rounded-md font-black text-[11px] text-white shadow-sm",
                      ...
                      <AccordionContent className="pb-3 pt-2 px-3 border-t border-primary/5 w-full bg-primary/[0.01]">
                        <div className="space-y-3">
                          <div className="flex items-center bg-white px-3 py-2 rounded-lg border border-primary/10 gap-2">
                            <Label className="text-[8px] font-black text-primary uppercase shrink-0">ENVÍO</Label>
                            <div className="relative flex-1">
                               ...
                               <Input 
                                  ...
                                  className="h-8 pl-6 text-xs font-black bg-primary/5 rounded border-none shadow-inner w-full" 
                                />
                            </div>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:bg-red-50 rounded-md shrink-0" onClick={() => setDeleteConfirm({ id: entry.customerId, name: entry.customerName })}>
          ...
          <div className="pt-6 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {historyBatches.map(batch => {
                ...
                let hDebt = false, hExcess = false, isZero = true;
                entries.forEach((e: any) => {
                  const b = calculateEntryBalance(e)
                  if (b < -0.1) { hDebt = true; isZero = false; }
                  else if (b > 0.1) { hExcess = true; isZero = false; }
                })

                return (
                  <Card key={batch.date} className="rounded-lg border border-black/5 bg-white shadow-sm hover:border-primary/20 transition-all cursor-pointer group" onClick={() => { setDate(new Date(batch.date + "T12:00:00")); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
                    <CardContent className="p-3 space-y-2">
                      ...
                      <div className="text-right border-t border-black/5 pt-1.5">
                         <span className="text-[10px] font-black text-primary mr-0.5">S/</span>
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <DialogContent className="rounded-[2rem] max-w-[280px] p-8 text-center border-none shadow-2xl bg-white">
          <div className="flex flex-col items-center gap-6">
             ...
             <p className="text-[10px] font-black uppercase text-black/60 leading-relaxed px-1">¿Remover a <span className="text-primary">{deleteConfirm?.name}</span> del lote?</p>
